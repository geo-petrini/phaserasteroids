...
export default class ShipHealth {
  constructor(scene, bodyWidth, shieldCapacity) {
    ...
    this.shield_hb = new HealthBar({ scene, width: 32, height: 4, fill_color: 0x00ffff, border_color: 0x00000000, max_value: shieldCapacity || 100 })
  }
  ...
  damage(amount) {
    if (this.shield_hb.value > 0) {
      const absorbed = Math.min(this.shield_hb.value, amount)
      this.shield_hb.decrease(absorbed)
      amount -= absorbed
    }
    if (amount > 0) {
  ...
  rechargeShield(amount, energyHb) {
    if (energyHb.value >= 1 && this.shield_hb.value < this.shield_hb.max_value) {
      energyHb.decrease(1)
      this.shield_hb.increase(amount)
    }
  }
