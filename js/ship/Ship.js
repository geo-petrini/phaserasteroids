...
export default class Ship extends Phaser.GameObjects.Sprite {
  ...
  constructor(config) {
    ...
    // ---- subsystems ----
    this.health = new ShipHealth(config.scene, this.body.width)
    ...
    this.targeting = new ShipTargeting(config.scene, this.weapons.trackWeaponOffsets, this.weapons.trackTurretAngles, this)
  ...
  /** Called from GameScene once the bullet group is created. */
  ...
  update(keys, time, delta) {
    ...
    if (keys.fire.isDown && time > this.lastFired) {
      const nextF = this.weapons.fireFixed(
        this.x, this.y, this.rotation, this.scaleX,
        this.bullets, time, this.FIRE_INTERVAL,
        this.health.energy_hb, this.config.scene.sounds,
        this.config.scene.options.volume_bullets
      )
      const nextT = this.weapons.fireTracking(
        this.x, this.y, this.rotation, this.scaleX,
        this.bullets, time, this.FIRE_INTERVAL,
        this.health.energy_hb, this.config.scene.sounds,
        this.config.scene.options.volume_bullets
      )
      this.lastFired = Math.min(
        nextF ?? Infinity,
        nextT ?? Infinity
      )
    }
